...
namespace s52
{
    ...
    static String getColorTableName(RenderSettings::ConstPtr settings){
        ...
        if (it == colorSchemes.end()) it=colorSchemes.begin();
        return it->second;
    }
    ...
    bool Attribute::equals(const String &lupv) const{
        ...
        if (lupv.empty() || lupv == " ") return true;
        if (lupv == "?") return false;
        switch(this->type){
            case T_INT:
            ...
            case T_DOUBLE:
                {
                    ...
                    if (::fabs(cmp-dv) < 1e-6) return true;
                }
            break;
            case T_STRING:
    ...
    int LUPrec::attributeMatch(const Attribute::Map *objectAttributes) const{
        ...
        for (auto it=ATTCArray.begin();it != ATTCArray.end();it++){
            ...
            if (!oit->second.equals(it->second)) return 0; 
            rt++;
        }
        return rt;
    }
    S52Data::S52Data(RenderSettings::ConstPtr settings, FontFileHolder::Ptr f, int s):
        sequence(s), pool(ocalloc::makePool("s52Data")),pref(pool),renderSettings(settings),fontFile(f){
    ...
    FontManager::Ptr S52Data::getFontManager(FontType type, int rsz) const{
        ...
        if (it == fontManagers.end()) return fontManagers.begin()->second;
        return it->second;
    }
    ...
    uint16_t S52Data::getAttributeCode(const String &name) const{
        ...
        if (rt == 0) throw AvException(FMT("attribute name %s not found in map",name));
        return rt;
    }
    const LUPrec *S52Data::findLUPrecord(const LUPname &name,uint16_t featureTypeCode,const Attribute::Map *attributes) const{
        ...
        if (it == lupTables.end()) return NULL;
        if (it->second.size() == 0) return NULL;
        auto oc=S57ObjectClasses::getObjectClass(featureTypeCode);
        if (oc == nullptr) return NULL;
        String acronym=oc->Acronym;
        const LUPrec *noAttrMatch=nullptr;
        const LUPrec *bestMatch=nullptr;
        int score=0;
        for (auto lui=it->second.begin();lui != it->second.end();lui++){
    ...
    void S52Data::addLUP(const LUPrec &lup){
        ...
        if (it == lupTables.end()){
            LUPRecords records;
            records.push_back(lup);
            lupTables[lup.TNAM]=records;
        }
        else{
            avnav::erase_if(it->second,[lup](const LUPrec& item){
                return item.RCID==lup.RCID;
            });
            it->second.push_back(lup);
        }
    }
    ...
    {
        AVASSERT((!froozenRules),"rules froozen");
        ruleCreator->rulesFromString(&lup,lup.INST,this,[&lup](const s52::Rule *r){
    ...
    void S52Data::buildRules(){
        ...
        AVASSERT((!froozenRules),"rules froozen");
        for (auto table=lupTables.begin();table != lupTables.end();table++ ){
            for (auto lup=table->second.begin();lup != table->second.end();lup++){
    ...
    void S52Data::createRasterSymbols(const String &dataDir){
        ...
        if (it == colorTables.end()) it=colorTables.begin();
    ...
    double S52Data::convertSounding(double valMeters, uint16_t attrid) const{
        if (attrid == 0
            || attrid == S57AttrIds::VERCLR 
            || attrid == S57AttrIds::VERCCL
            || attrid == S57AttrIds::VERCOP
            || attrid == S57AttrIds::HEIGHT
            || attrid == S57AttrIds::HORCLR
