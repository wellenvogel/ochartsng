...
void DrawingContext::drawLine(const Coord::PixelXy &p0, const Coord::PixelXy &p1, const DrawingContext::ColorAndAlpha &color, bool useAlpha, const DrawingContext::Dash *dash, bool overlap){
    ...
    //do the steps in the order max/min
    int ystepRound=majorX?1:0;
    int xstepRound=majorX?0:1;
    for (int stepRound = 0; stepRound < 2; stepRound++)
    {
        if (stepRound == ystepRound)
        {
            if (y < 0)
            {
                ...
                if (error > dy)
                {
                    Coord::Pixel stepx = -(2 * step - 1) * dx / dy2;
        ...
        if (stepRound == xstepRound)
        {
            ...
            if (stepx)
            {
                ...
                if (error < dx)
                {
                    Coord::Pixel stepy = -(2 * stepx - 1) * dy / dx2;
    ...
    if (y2 >= height) y2=height-1; //stop there, avoid separate check
    if (sign_x > 0){
